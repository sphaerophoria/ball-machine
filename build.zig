...
fn buildCChamber(b: *std.Build, chambers_step: *std.Build.Step, libphysics: *std.Build.Step.Compile, opt: std.builtin.OptimizeMode) !void {
    //zig cc -target wasm32-freestanding -Oz -I../libphysics -o simple2.wasm simple2.c walloc.c -Wl,--no-entry -Wl,--export=alloc -Wl,--export=init -Wl,--export=deinit -Wl,--export=free -Wl,--export=save -Wl,--export=load -Wl,--export=step -Wl,--export=render -Wl,--export=saveSize -lphysics -L ../../zig-out/lib/
    ...
    const files = [_][]const u8{"simple2.c"};
