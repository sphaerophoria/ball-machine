...
const State = struct {
    ...
    fn pushPosition(self: *State, pos: physics.Pos2) void {
        ...
        self.x_history[self.idx % len] = @intFromFloat(pos.x * std.math.maxInt(@TypeOf(self.x_history[0])));
        self.y_history[self.idx % len] = @intFromFloat(pos.y / chamber_height * std.math.maxInt(@TypeOf(self.y_history[0])));
        self.idx = (self.idx + 1);
        if (self.idx > len * 2) {
